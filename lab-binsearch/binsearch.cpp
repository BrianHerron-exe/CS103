...
int* r_min(int*data, int* min, int len);
...
int binsearch(int target, int* data, int start, int end)
{
  ...
  if(start+1 < end){
    ...
    cout << "the value " << target << "is not found" << endl;
  }
  return -1;
}
...
if(len==1){
  ...
  find min index, swap with data[0]
  sort(data+1, len-1);
}
*/
...
int* r_min(int*data, int* min, int len){
  if(len==1){
    return ((data[0] < *min) ? data : min);
  }
  else{
    int* t = r_min(data+1, min, len-1);
    if(*data < *t){
      min = data;
      return data;
    }
