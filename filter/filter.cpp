...
void sobel(unsigned char out[][SIZE][RGB], unsigned char in[][SIZE][RGB]){
	...
	for(int i=0 ; i<SIZE ; i++){
		for(int j=0 ; j<SIZE ; j++){
			for(int c=0 ; c<RGB ; c++){
				tempNum = h1_soble[i][j][c] + h2_soble[i][j][c];
				if(tempNum > 255){
					tempNum = 255;
				}
				if(tempNum < 0){
				...
				out[i][j][c] = (unsigned char)tempNum;
			}
		}
	} 
}
...
int N, double sigma, double alpha){
	...
	for(int i=0 ; i<SIZE ; i++){
		for(int j=0 ; j<SIZE ; j++){
			for(int k=0 ; k<RGB ; k++){
				tempNum = in[i][j][k] - temp[i][j][k];
				tempNum = (int) (in[i][j][k] + alpha*tempNum);
				if(tempNum > 255){
					tempNum = 255;
				}
				else if(tempNum < 0){
				...
				out[i][j][k] = (unsigned char)tempNum;
			}
		}
	}
	...
	// 	for(int j=0 ; j<SIZE ; j++){
	// 		for(int k=0 ; k<RGB ; k++){
	// 			out[i][j][k] = in[i][j][k] + alpha*temp[i][j][k];
	// 			if(temp[i][j][k] > 255){
	// 				temp[i][j][k] = 255;
	// 			}
	// 			else if(temp[i][j][k] < 0){
